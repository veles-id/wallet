...
async function initialize() {
  ...
  app.get('/api/auth/options', async (req, res) => {
    try {
      ...
      /**
       * @todo
       * Store the challenge temporarily - in a future version, associate it with the user during verification
       * For now, store it in a simple way (in production, use a proper session store)
       */
  ...
  app.post('/api/register/verify', async (req, res) => {
    try {
      ...
      if (verification.verified && verification.registrationInfo) {
        ...
        console.log('User after device addition:', await userStore.getUserById(user.id));
  ...
  app.post('/api/auth/verify', async (req, res) => {
    try {
      ...
      console.log('Received authentication verification request:', JSON.stringify(credential, null, 2));
      ...
      const authenticator = user.devices.find((dev) => dev.credentialID === credential.rawId);

      if (!authenticator) {
        console.log('Authenticator not found for credential ID:', credential.rawId);
        console.log(
      ...
      console.log('Verifying authentication with:', {
        ...
        authenticator: {
          ...
          credentialPublicKey: authenticator.credentialPublicKey ? 'present' : 'missing',
      ...
      console.log('authenticatorForVerification:', {
        ...
        credentialPublicKey: authenticatorForVerification.credentialPublicKey ? 'Buffer present' : 'missing',
        credentialPublicKeyLength: authenticatorForVerification.credentialPublicKey?.length,
